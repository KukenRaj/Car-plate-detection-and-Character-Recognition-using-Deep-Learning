...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):  # Plots one bounding box on image img
    ...
    c1, c2 = (int(x[0])-14, int(x[1])-14), (int(x[2])+14, int(x[3])+14)
    imgcrp = img[(int(x[1])-6):(int(x[3])+6), (int(x[0])-6):(int(x[2])+6)]
    imgnww = cv2.copyMakeBorder(imgcrp, 30, 30, 30, 30, cv2.BORDER_CONSTANT)
    #RGBLOWER = (0, 0, 0)
    #RGBUPPER = (106, 255, 255)
    #mask = cv2.inRange(imgnww, RGBLOWER, RGBUPPER)
    gray = cv2.cvtColor(imgnww, cv2.COLOR_BGR2GRAY)
    (thresh,mask) = cv2.threshold(gray,160,255,cv2.THRESH_BINARY_INV)
    mask = cv2.threshold(gray, 160, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
    text = pytesseract.image_to_string(mask)
    x = re.findall("^[A-Z]{1,10} [0-9]{1,4}$|^[A-Z]{1,10}\n[0-9]{1,4}$|^[A-Z]{1,10} [0-9]{1,4} [A-Z]{1,10}$|^[A-Z]{1,10}[0-9]{1,4}[A-Z]{1,10}$", text)
    #cv2.imwrite('/content/warpimages/'+"image"+"("+str(label)+")"+".jpg", mask)
    if x:
        tf = max(tl - 1, 1)  # font thickness
        cv2.rectangle(img, c1, c2, color, thickness=tl)
        listt.append(text)
        result = checkIfDuplicates_2(listt)
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1)  # filled
        cv2.putText(img, (pytesseract.image_to_string(mask)), (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        txxt = "{}".format(statt)
        cv2.putText(img, txxt, (10, 90),cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 2)
