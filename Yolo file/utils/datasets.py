...
class LoadWebcam:  # for inference
    def __init__(self, video_path, img_size=360):
        self.cam = cv2.VideoCapture(video_path)
        self.height = img_size
        prop = cv2.cv.CV_CAP_PROP_FRAME_COUNT if imutils.is_cv2() \
        else cv2.CAP_PROP_FRAME_COUNT
        self.nF = int(self.cam.get(prop))
    ...
    def __next__(self):
        ...
        pts2 = np.float32([[0,0],[0,H],[W,0],[W+100,H+300]])
        matrix = cv2.getPerspectiveTransform(pts1, pts2)
        result = cv2.warpPerspective(img0, matrix, (W, H))
        if ret_val:
          frame = result[600:1600, 800:1800]
        img_path = 'outcomeimage_%g.jpg' % self.count

        # Padded resize
        img, _, _, _ = letterbox(frame, height=self.height)
...
class LoadImagesAndLabels:  # for training
    def __init__(self, path, batch_size=1, img_size=608, multi_scale=False, augment=False):
        with open(path, 'r') as file:
            ...
            self.label_files = [x.replace('\n', '') for x in self.label_files]
            self.label_files = list(filter(lambda x: len(x) > 0, self.label_files))

        self.img_files = [x.replace('.txt', '.png')
                            for x in self.label_files]
        # self.img_files = list(glob.glob(os.path.join(path,"*.jpg")))
        # self.label_files = list(glob.glob(os.path.join(path,"*.txt"))) 
        ...
        for index, files_index in enumerate(range(ia, ib)):
            ...
            if os.path.isfile(os.path.join("labels_txt",label_path)):
                with warnings.catch_warnings():
                    ...
                    labels0 = np.loadtxt(os.path.join("labels_txt",label_path), dtype=np.float32).reshape(-1, 5)
